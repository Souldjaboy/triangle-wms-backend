#!/usr/bin/env bash
# Calendrier administratif : jours fériés, chômés, exceptionnels. Base vierge → verdict.
# De la base vierge au verdict, en une commande. Aucun paiement n'est déclenché.
set -euo pipefail
PGBIN=${PGBIN:-/usr/lib/postgresql/16/bin}; PGPORT=${PGPORT:-5433}
PGDATA=${PGDATA:-/var/lib/postgresql/pgtest}; BASE=${BASE:-wms_jours_chomes}
RACINE="$(cd "$(dirname "$0")/.." && pwd)"
export DATABASE_URL="postgresql://postgres@127.0.0.1:${PGPORT}/${BASE}"
export JWT_SECRET="${JWT_SECRET:-test_chomes}" FRONTEND_URL="${FRONTEND_URL:-http://localhost:3000}" NODE_ENV=test
...
echo "── serveur"
cd "$RACINE"; node server.js > /tmp/serveur-chomes.log 2>&1 &
SRV=$!; trap 'kill $SRV 2>/dev/null || true' EXIT
for _ in $(seq 1 40); do curl -s -o /dev/null http://localhost:5050/ 2>/dev/null && break; sleep 1; done

echo "── tests"
node "$RACINE/scripts/test-jours-feries-chomes.js"
