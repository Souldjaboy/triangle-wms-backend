#!/usr/bin/env bash
# L'écran de paie dans un vrai navigateur, en 1280 px et 375 px : les cinq
# situations de droits, dont celle d'un backend pas encore mis à jour.
set -euo pipefail
PGBIN=${PGBIN:-/usr/lib/postgresql/16/bin}; PGPORT=${PGPORT:-5433}
PGDATA=${PGDATA:-/var/lib/postgresql/pgtest}; BASE=${BASE:-wms_droits_nav}
RACINE="$(cd "$(dirname "$0")/.." && pwd)"
FRONT=${FRONT:-$(cd "$RACINE/../triangle-wms-frontend" && pwd)}
export DATABASE_URL="postgresql://postgres@127.0.0.1:${PGPORT}/${BASE}"
export JWT_SECRET="${JWT_SECRET:-test_droits}" FRONTEND_URL="http://localhost:3000" NODE_ENV=test
...
echo "── backend"
cd "$RACINE"; node server.js > /tmp/serveur-droits-nav.log 2>&1 &
SRV=$!
...
BASE=http://localhost:3000 BACKEND=http://127.0.0.1:5050 SECRET="$JWT_SECRET" \
  SORTIE="${SORTIE:-/tmp/captures-paie-droits}" node scripts/verif-paie-droits.js
