...
PGDATA=${PGDATA:-/var/lib/postgresql/pgtest}; BASE=${BASE:-wms_sept_test}
RACINE="$(cd "$(dirname "$0")/.." && pwd)"
export DATABASE_URL="postgresql://postgres@127.0.0.1:${PGPORT}/${BASE}"
export JWT_SECRET="${JWT_SECRET:-test_sept}" FRONTEND_URL="${FRONTEND_URL:-http://localhost:3000}" NODE_ENV=test
...
echo "── serveur"
cd "$RACINE"; node server.js > /tmp/serveur-sept.log 2>&1 &
SRV=$!; trap 'kill $SRV 2>/dev/null || true' EXIT
for _ in $(seq 1 40); do curl -s -o /dev/null http://localhost:5050/ 2>/dev/null && break; sleep 1; done

echo "── tests"
node "$RACINE/scripts/test-paie-septembre-exception.js"
