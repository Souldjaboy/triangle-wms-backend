#!/usr/bin/env bash
# Primes, heures supplémentaires, non rémunéré, recalcul idempotent.
# Base vierge à chaque exécution. Aucun paiement déclenché.
set -euo pipefail
PGBIN=${PGBIN:-/usr/lib/postgresql/16/bin}; PGPORT=${PGPORT:-5433}
PGDATA=${PGDATA:-/var/lib/postgresql/pgtest}; BASE=${BASE:-wms_primes_test}
RACINE="$(cd "$(dirname "$0")/.." && pwd)"
export DATABASE_URL="postgresql://postgres@127.0.0.1:${PGPORT}/${BASE}"
export JWT_SECRET="${JWT_SECRET:-test_primes}" FRONTEND_URL="${FRONTEND_URL:-http://localhost:3000}" NODE_ENV=test
...
echo "── serveur"; cd "$RACINE"; node server.js > /tmp/serveur-primes.log 2>&1 &
SRV=$!; trap 'kill $SRV 2>/dev/null || true' EXIT
for _ in $(seq 1 40); do curl -s -o /dev/null http://localhost:5050/ 2>/dev/null && break; sleep 1; done

echo "── tests"; node "$RACINE/scripts/test-primes-heures-sup.js"
